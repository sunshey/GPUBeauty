...
const std::string kGPUImageBaseBeautyFaceVertexShaderString = R"(
    ...
    void main(void) {
      ...
      textureShift_3 = vec4(inputTextureCoordinate + vec2(widthOffset, heightOffset),
...
//    const float alpha = 0.7;
    ...
    const float   levelRangeInv = 1.060816;
    const float   levelBlack = 0.01250784;
...
//        float theta = 0.1;
        float theta = 0.3;
        float p =
            clamp((min(iColor.r, meanColor.r - 0.1) - 0.05) * 5.0, 0.0, 1.0);
