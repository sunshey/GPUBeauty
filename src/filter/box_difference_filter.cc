...
const std::string kBoxDifferenceFragmentShaderString = R"(
    ...
    void main() {
      ...
      diffColor = min(diffColor * diffColor, 1.0);
        ...
        // 高通滤波之后的颜色值
//        highp vec3 highPassColor = iColor - meanColor;
//        // 对应混合模式中的强光模式(color = 2.0 * color1 * color2)，对于高反差的颜色来说，color1 和color2 是同一个
//        highPassColor.r = clamp(2.0 * highPassColor.r * highPassColor.r * delta, 0.0, 1.0);
//        highPassColor.g = clamp(2.0 * highPassColor.g * highPassColor.g * delta, 0.0, 1.0);
//        highPassColor.b = clamp(2.0 * highPassColor.b * highPassColor.b * delta, 0.0, 1.0);
