...
NS_GPUPIXEL_BEGIN
 ...
 void main()
 {
    ...
    bool FaceReshapeFilter::init() {
        ...
        registerProperty("thin_face", 0, "The smoothing of filter with range between -1 and 1.",
                         [this](float &val) {
                             setFaceSlimLevel(val);
                         });

        registerProperty("big_eye", 0, "The smoothing of filter with range between -1 and 1.",
                         [this](float &val) {
                             setEyeZoomLevel(val);
                         });
